...
export default function HouseList(){
  ...
  const filtered = useMemo(()=>{
    ...
    return HOUSES.filter(h => {
      ...
      const priceOk = (!flt.minPrice || h.price >= flt.minPrice) && (!flt.maxPrice || h.price <= flt.maxPrice)
      const areaOk = (!flt.minArea || h.area >= flt.minArea)
      const bedsOk = (!flt.minBeds || h.beds >= flt.minBeds)
      const bathsOk = (!flt.minBaths || h.baths >= flt.minBaths)
      return qmatch && cmatch && priceOk && areaOk && bedsOk && bathsOk
    })
  }, [query, cats, flt])
  ...
  return (
    <div className="space-y-6">
      ...
      <FilterBar state={flt} onChange={setFlt} />
