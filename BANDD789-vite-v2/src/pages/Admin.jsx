...
function buildSitemapXML(base, articles, projects){
  const staticUrls = ['/', '/แบบบ้าน', '/ผลงาน', '/รีวิวลูกค้า', '/คำถามที่พบบ่อย', '/บทความ']
  const articleUrls = (articles||[]).map(a => '/บทความ/' + encodeURIComponent(a.slug))
  const projectUrls = (projects||[]).map(p => '/ผลงาน/' + encodeURIComponent(p.slug))
  const urls = staticUrls.concat(articleUrls).concat(projectUrls)

  const SEP = String.fromCharCode(10) // newline
  const parts = []
  parts.push('<?xml version="1.0" encoding="UTF-8"?>')
  parts.push('<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">')
  for (let i = 0; i < urls.length; i++) {
    parts.push('  <url><loc>' + base + urls[i] + '</loc></url>')
  }
  parts.push('</urlset>')
...
`)
  ...
  // removed old template literal due to build errors
// // [removed legacy template literal XML block]  <url><loc>${base}${u}</loc></url>`).join('\n')}
</urlset>`
  return xml
}
function buildRSSXML(base, articles){
  ...
  // removed old template literal due to build errors
// // [removed legacy template literal XML block]
  return xml
}
...
<button className="btn-outline" onClick={() => {
  ...
  const base = (baseEl && baseEl.value) ? baseEl.value : window.location.origin
  const xml = buildSitemapXML(base, listArticles(), listProjects())
