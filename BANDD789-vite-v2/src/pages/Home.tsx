...
import { FEATURED_HOUSES } from '../store/houses'
    ...
    <>
      ...
      <div className="container">
        ...
        <section className="section">
          <h2 style={{marginTop:0}}>แบบบ้านแนะนำ</h2>
          <p className="muted" style={{marginTop:-6}}>เลือกชมแบบบ้านยอดนิยมจาก BANDD789</p>
          <div className="cards">
            {FEATURED_HOUSES.map(h => (
              <article className="card" key={h.id} aria-label={`house-${h.name}`}>
                ...
                <div style={{display:'flex', justifyContent:'space-between', alignItems:'center'}}>
                  ...
                  <div className="muted" style={{fontSize:12}}>{h.category}</div>
                </div>
                <h3 style={{margin:'8px 0 4px'}}>{h.name}</h3>
                <div className="muted">พื้นที่ {h.area} ตร.ม. • {h.beds} นอน • {h.baths} น้ำ • จอด {h.parking}</div>
                <div style={{fontWeight:800, color:'var(--primary-700)', marginTop:6}}>{h.price.toLocaleString()} บาท</div>
              </article>
            ))}
          </div>
          <div style={{display:'flex', justifyContent:'center', marginTop:16}}>
            <a href="/แบบบ้าน"><button>ดูแบบบ้านทั้งหมด</button></a>
          </div>
        </section>
